...
def run_crawler(spider):
    if spider['spider'] == ReviewSpider:
        # Loading products from json
        with open('products.json') as f:
            products = json.load(f)
        spider['arguments']['products'] = products
...
if __name__ == "__main__":
    ...
    if not ip:
        ...
        process = CrawlerProcess(settings=reviews_settings)
        process.crawl(ReviewSpider, products=[{'product_id' : re.findall(r'[0-9]+', url)[-1]}])
        process.start()